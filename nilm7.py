...
import sys
sys.path.insert(0,'C:/Users/wesin/Documents/Doutorado/nilmtk/nmf/wa_nmf')
from nmf import *
...
#############
alpha = 0.012
model = NonnegativeMatrixFactorization(n_components=len(appliances_value),alpha=alpha, max_iter=1000, solver='mu')

############
#train step
############
D,A = model.initialize(aggregate_signal,type=2)
newD, newA = model.train(aggregate_signal,D,A)
train_error = _beta_divergence(aggregate_signal,newD,newA, beta = 2, square_root=True)
...
#evaluate step
##############
# investigate newD utilization in test phase
test_error = _beta_divergence(test_signal, newD, newA, beta = 2, square_root=True)
...
for appliance in appliances_value:
	...
	wi = newD[:,k].reshape(n_sample,1)
	hi = newA[k,:].reshape(1,n_day)
